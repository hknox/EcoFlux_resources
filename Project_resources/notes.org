...
Fix base urls.py in prod for MEDIA
...
** DONE Create a Base Settings File

You can create a base settings file and then extend it for different
environments.

*** Create a `settings` directory
Inside your Django project, create a directory named `settings`.

*** Create `base.py`
This file will contain common settings.
   ```python
   # settings/base.py
   DEBUG = False
   ALLOWED_HOSTS = []
...
These files will import from `base.py` and override specific settings.
...
When running your application, specify which settings file to use.
   ```bash
