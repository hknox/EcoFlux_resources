...
change {% if cancel_url %} to {% if action == 'update' %} or something


** Upload documents
** Start exploring deployment


** Photos
** If Equipment.location == None, display as something else?
** Review Filters
...
{% if...
<!-- -->
** clean up tooltip stuff in forms
...
X Add notes, linked to each history record
X Field notes linked to site
X No supplier: encode mfr/supplier in model number field
